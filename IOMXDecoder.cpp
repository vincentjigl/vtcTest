    ...
    switch (eEvent) {
            ...
            if (nData1 == OMX_CommandPortDisable) {
                ...
                if (nData2 == OMX_DirOutput) {
                    ...
                    if (mPortReconfigInProgress) {
                        ...
                        if(err!=OK)
							printf("sendCommand(mNode, OMX_CommandPortEnable, OUTPUT_PORT) error");
        ...
        case OMX_EventPortSettingsChanged:
            if (nData2 == 0 || nData2 == OMX_IndexParamPortDefinition) {
                ...
                if(err!=OK)
					printf("OMX_CommandPortDisable error ");
...
status_t OMXDecoder::FillBufferDone(IOMX::buffer_id pBufferHdr) {
    ...
    err = mNativeWindow->queueBuffer(mNativeWindow.get(), mOutputBuffers[i].gb.get(), -1);
    if (err != 0) {
...
status_t OMXDecoder::createPlaybackSurface() {
    ...
    if(mSurfaceComposerClient->initCheck()!=OK)
...
status_t OMXDecoder::allocateOutputBuffersFromNativeWindow() {
    ...
    for (OMX_U32 i = cancelStart; i < cancelEnd; i++) {
        ...
        err = mNativeWindow->cancelBuffer(
                mNativeWindow.get(), info->gb.get(), -1);
        if (err != 0) {
...
status_t OMXDecoder::freeOutputBuffers() {
    ...
    for (size_t i = mOutputBuffers.size(); i-- > 0;) {
        ...
        if (mOutputBuffers[i].mStatus == OWNED_BY_US && mOutputBuffers[i].gb != 0) {
            err = mNativeWindow->cancelBuffer(mNativeWindow.get(), mOutputBuffers[i].gb.get(), -1);
