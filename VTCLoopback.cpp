 ...
 */
#define ANDROID_API_JB_OR_LATER 
...
uint32_t gCameraSurfaceWidth = 640;
uint32_t gCameraSurfaceHeight = 480;
...
int createPreviewSurface() {
    ...
    if(gSurfaceComposerClient->initCheck()!=OK)
		printf("gSurfaceComposerClient->initCheck()!=OK");
	
    DisplayInfo dInfo;
	gSurfaceComposerClient->getDisplayInfo(0, &dInfo);

    //gCameraSurfaceWidth = dInfo.w;
    //gCameraSurfaceHeight = dInfo.h;
	printf("gCameraSurfaceWidth %d; gCameraSurfaceHeight %d \n",gCameraSurfaceWidth, gCameraSurfaceHeight );

    gSurfaceControl = gSurfaceComposerClient->createSurface(String8("previewSurface"),
            gCameraSurfaceWidth,
...
int configureCamera() {
    ...
    gICamera->setPreviewTarget(gPreviewSurface->getIGraphicBufferProducer());
...
int configureCamera() {
    ...
    createPreviewSurface();
    gICamera = Camera::connect(gCameraIndex, String16(), -1);
    if (gICamera.get() == NULL){
        VTC_LOGE("camera.get() =================== NULL");
        return -1;
    }

    VTC_LOGD("\n\n mPreviewWidth = %d, mPreviewHeight = %d, mVideoFrameRate = %d, mVideoBitRate = %d \n\n\n",
        gPreviewWidth, gPreviewHeight, gCameraSurfaceWidth, gCameraSurfaceHeight);

    String8 param_str = gICamera->getParameters();
    CameraParameters params(param_str);

    params.unflatten(gICamera->getParameters());
    params.setPreviewSize(gPreviewWidth, gPreviewHeight);
    params.setPreviewFrameRate(gCameraFrameRate);
    params.set(CameraParameters::KEY_RECORDING_HINT, CameraParameters::TRUE);// Set recording hint, otherwise it defaults to high-quality and there is not enough memory available to do playback and camera preview simultaneously!
    sprintf(mParamValue,"%u,%u", gCameraFrameRate*1000, gCameraFrameRate*1000);
    ...
    gCameraListener = new MyCameraListener();
    gICamera->setListener(gCameraListener);
...
int test_DEFAULT_Frame() {
    ...
    OMXClient omxclient;
    if(omxclient.connect()!=OK)
		printf("omxclient.connect()!=OK");
