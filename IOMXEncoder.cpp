...
status_t OMXEncoder::configure(OMX_VIDEO_AVCPROFILETYPE profile, OMX_VIDEO_AVCLEVELTYPE level, OMX_U32 refFrames) {
    ...
    err = mOMX->getParameter(mNode, OMX_IndexParamVideoProfileLevelQuerySupported, &profileLevel, sizeof(profileLevel));
    if (err != OK) {
...
status_t OMXEncoder::start(MetaData *params) {
	...
	while(mState != OMX_StateExecuting){
    ...
    for (int i=0; i<(int)tOutPortDef.nBufferCountActual; i++) {
        ...
        if (err != OK) {
            ...
            VTC_LOGD("called fillBuffer(%d)\n ",i);
        }
    }
	VTC_LOGD("OMX_FillThisBuffer success \n", err);
	...
	VTC_LOGD("OMX_emptyBuffer begin \n", err);
    ...
    for (int i=0; i<3; i++) {
        ...
        if (err != OK) {
            ...
            VTC_LOGD("Called EmptyThisBuffer[%d] \n",i);
        }
    }
	VTC_LOGD("OMX_emptyBuffer success \n", err);
...
status_t OMXEncoder::stop() {
    ...
    if (mSliceHeight == 0) { // non tunnel mode
        ...
        if (err != OK) {
            VTC_LOGD("OMX_CommandFlush for input port (0) failed:%d\n", err);
        }
    }

    err = mOMX->sendCommand(mNode,OMX_CommandFlush, OUTPUT_PORT);
    if (err != OK) {
        VTC_LOGD("OMX_CommandFlush for output port (1) failed:%d\n", err);
    }
...
OMX_ERRORTYPE OMXEncoder::EventHandler(OMX_EVENTTYPE eEvent, OMX_U32 nData1,OMX_U32 nData2) {
    ...
    VTC_LOGV("########## EventHandler: eEvent:0x%x, nData1:%d, nData2:0x%x, pid=%d \n", (int)eEvent, (int)nData1, (int)nData2, getpid());
    ...
    switch (eEvent) {
        case OMX_EventCmdComplete:
            if (nData1 == OMX_CommandPortDisable) {
                ...
                VTC_LOGD( "Component OMX_EventCmdComplete OMX_CommandStateSet new State:%s \n",OMXStateName((OMX_STATETYPE)nData2));
                {
					VTC_LOGD( "EventHandler autoLock begin \n");
                    //Mutex::Autolock autoLock(mLock);
					VTC_LOGD( "EventHandler autoLock end \n");
                    mState = (OMX_STATETYPE)nData2;
                }
                VTC_LOGD( "signal begin \n");
                mAsyncCompletion.signal();
                VTC_LOGD("signal end \n");

            } else if (nData1 == OMX_CommandFlush) {
                VTC_LOGD( "Component OMX_EventCmdComplete OMX_CommandFlush port:%d \n",(int)nData2);
            } else {
                VTC_LOGD( "Component OMX_EventCmdComplete command:%d\n",(int)nData1);
            }
            break;
...
status_t OMXEncoder::FillBufferDone(OMX_BUFFERHEADERTYPE* pBufferHdr, OMX_U32 nOffset, OMX_U32 nFilledLen, OMX_TICKS nTimeStamp) {
    ...
    if (mDebugFlags & DEBUG_DUMP_ENCODER_TIMESTAMP) VTC_LOGD("FBD TS: %lld\n", nTimeStamp);
...
status_t OMXEncoder::waitForStateSet(OMX_STATETYPE newState) {
    VTC_LOGV("waitForStateSet: Waiting to move to state %s .....\n", OMXStateName(newState));
    ...
    //status_t retval = mAsyncCompletion.waitRelative(mLock, TWO_SECOND);
    	{
    status_t retval = mAsyncCompletion.wait(mLock);
